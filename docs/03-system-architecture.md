...
graph TB
    ...
    subgraph Data["Data Layer"]
        DB[(PostgreSQL — Neon<br/>no local database)]
    end
...
graph LR
    ...
    Vercel -->|prisma generate + migrate deploy<br/>against prisma/schema.prisma| Neon[(Neon Postgres)]
...
| **Compare-and-swap concurrency control, not row locking** | `claimNextRecordForUser()` | Avoids relying on a database-specific `SELECT ... FOR UPDATE` | Requires an explicit bounded retry loop (5 attempts) rather than the database blocking automatically |
...
| **One Postgres schema, no local database** | `prisma/schema.prisma`, `prisma/migrations/` | Local dev, tests, and production all share one schema and one migration history — no risk of the two drifting apart | Local dev normally points at the real (production) database via `.env.local`; there's no disposable local copy to experiment against, so schema changes should be reviewed carefully before applying |
