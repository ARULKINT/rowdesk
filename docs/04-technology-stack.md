...
| SQLite | (via `DATABASE_URL=file:...`) | Database | Local development + test database | `prisma/schema.prisma` |
| PostgreSQL | (Neon-hosted) | Database | Production database | `prisma/postgres/schema.prisma` |
...
| tsx | ^4.23.13 | Dev tool | Run TypeScript scripts directly (seeding) | `prisma/seed.ts` |
| dotenv | ^17.4.2 | Dev tool | Load `.env` for Prisma config | `prisma.config.ts` |
...
| **Prisma** | Type-safe queries and a single schema-driven migration workflow across both SQLite (dev) and PostgreSQL (prod). |
| **SQLite locally / PostgreSQL in production** | SQLite requires no local service to develop against; PostgreSQL is what Neon (the chosen managed database) provides. Prisma's `datasource.provider` is a static string, so this is implemented as two parallel schema files rather than one — see [18-deployment.md](18-deployment.md). |
